...
| 22 | **orca 모드 + `~/.local/bin/cmux` 구버전 shim 잔재 (2026-07-24 실측)** | Orca 터미널에서 bare `cmux` 가 소켓으로 **별도 실행 중인 cmux 앱**에 연결돼 엉뚱한 pane 을 조용히 조작(오발사). shim 이 ALL_HELPERS 에 없고 gap-fill 은 부재 시에만이라 구 shim 이 영원히 미갱신 | deft-bin-sync(claude-2.48.1~)가 기본 동기 시 **deft-cmux-shim 마커 보유 파일만** 갱신(진짜 cmux CLI 는 마커 검사로 보호). orca 검증은 ① 환경 판정 ORCA_* 우선 ② rebalance 계열 no-op·shim exit 1 확인 ③ spawn 은 `deft-claudex-native-spawn` orca 분기(split `--command` 원샷·`env:"orca"` JSON·tui-idle)·pane 은 종료 시 Orca 자동 close |
