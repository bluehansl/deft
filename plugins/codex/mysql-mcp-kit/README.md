...
MySQL/MariaDB를 **benborla MCP**(`@benborla29/mcp-server-mysql`)로 안전하게 조회하는 Codex 셋업 키트.
...
1. Codex 마켓플레이스에서 `mysql-mcp-kit` 설치.
2. `skills/db-access/README.md` 런북에 따라 래퍼 배치, 크리덴셜 입력, MCP 서버 등록, 권한 게이트, AGENTS.md 가이드를 설정.
...
| `bin/db-audit-log` | 감사 로그 스크립트(PostToolUse 훅을 쓰는 호스트용) |
