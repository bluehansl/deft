...
Codex에서 sub-agent를 사용할 수 있으면 reviewer별 one-shot 작업으로 병렬 실행한다.
...
sub-agent 실행이 불가능하면 Lead가 Bash로 각 CLI를 직접 실행한다. 가능한 경우 병렬로 실행하되, 출력 수집과 timeout 처리를 명확히 한다.
...
### AI별 검토 결과
- **Lead Codex**: 핵심 판단
- **Codex reviewer**: 핵심 판단 또는 skip 사유
- **Gemini**: 핵심 판단 또는 skip 사유
- **Claude**: 핵심 판단 또는 skip 사유
