...
`HAVE_CMUX=0` 이고 claudex MCP가 등록된 경우 사용하는 stateful 통신 경로. Codex가 lead일 때 MCP tool 호출 (실제 tool name은 codex 0.134.0의 MCP 명세에 따름 — 예: `claudex.codex` / `claudex.codex-reply`):
