...
#   wrapper 는 매 호출 env→표준경로로 진짜 cmux 해석 → 신·구 cmux + 환경 차이에 강함. (§2-3·§9-1 bare cmux 보호)
...
# cmux-rebalance-guard 헬퍼 설치 — Agent spawn 마다 cmux 재계산이 Lead 비율을 깎는 것(60%→26%, 실측)을
#   0.1초 폴링으로 ~1초 내 자동 교정. watch(settle 후 1회)보다 강력 — 매 spawn 재계산을 반복 교정 + grace 종료.
if ! command -v cmux-rebalance-guard >/dev/null 2>&1; then
  ...
  [ -n "$SRC" ] && mkdir -p ~/.local/bin && cp "$SRC" ~/.local/bin/ && chmod +x ~/.local/bin/cmux-rebalance-guard
fi
