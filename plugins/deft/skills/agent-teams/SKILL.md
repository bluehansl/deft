...
> 🟠 **orca 모드**: 아래 tmux 블록을 실행하지 않는다 — orca 모드의 bare `tmux`/`cmux` 호출은 금지(오발사·shim 동작 미검증). claude 팀원은 in-process 라 ①의 graceful 종료가 유일 수단인 것은 동일하고, 정상 종료 시 pane 은 Orca 가 정리한다. **잔존 orphan pane 은 사용자에게 UI 수동 닫기를 안내**한다.
```bash
