...
> 🟠 **orca 모드 (§0-1 환경 판정)**: 본 절 전체를 **skip** 한다 — Orca 는 resize CLI 미지원(`orca terminal resize` 부재)이라 rebalance 자체가 불가하고, 워처/guard 는 cmux CLI 기반이라 오발사 위험(내장 가드가 no-op 처리). pane 배치는 orca claude-teams 가 자동으로 하며, **비율이 마음에 안 들면 UI 드래그로 조정하라고 사용자에게 안내**한다. `LEAD_REF`/`BASE` 캡처(`cmux identify`/`tmux list-panes`)도 하지 않는다.
...
> 🟠 **orca 모드**: 아래 tmux 블록을 실행하지 않는다 — orca 모드의 bare `tmux`/`cmux` 호출은 금지(오발사·shim 동작 미검증). claude 팀원은 in-process 라 ①의 graceful 종료가 유일 수단인 것은 동일하고, 정상 종료 시 pane 은 Orca 가 정리한다. 잔존 orphan pane 은 `orca terminal list --worktree active --json` 에서 해당 팀원 pane 의 handle 을 확인 후 `orca terminal close --terminal <handle>` 로 닫거나(확신 없으면) **사용자에게 UI 수동 닫기를 안내**한다.
```bash
