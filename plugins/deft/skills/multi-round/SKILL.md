...
> 🚨 **cmux send 3대 규약 (zsh 환경 실측 — 모두 준수)** (근거: RATIONALE R-15):
> 1. **send 전 입력창 클리어** — 매 `cmux send` 직전 `cmux send-key --surface "$SURF" "${WS[@]}" C-u` 를 먼저 보낸다. cmux send 는 입력창에 텍스트를 쌓는데, 직전 send 잔여물·send-key Enter 미전달 잔재가 남아 명령이 오염된다(실측: `touch` 텍스트가 prompt 에 박힌 채 실행 안 됨).
> 2. **긴 명령은 `.sh` 파일 + `source`** — MCP 인라인(`-c 'mcp_servers.bus={…}'`)이 붙은 부팅 명령은 수백 자라 cmux send 한 번에 셸에 도달 못 하고 유실된다. **`$SESSION_DIR/boot-<name>.sh` 에 저장 → pane 엔 `source $SESSION_DIR/boot-<name>.sh` 한 줄만 send**(짧은 명령만 직접 send).
> 3. **colon ref·zsh 파싱 회피** — `surface:N` 의 콜론이 `${pair%%:*}` 같은 파싱을 깨고, zsh 위치파라미터(`set -- $row; $1`)는 비표준이라 어긋난다. **루프·구분자 파싱으로 영리하게 짜지 말고, 워커별로 surface ref 를 명시적으로 나열**해 처리한다.

```bash
SURF="$W1_SURFACE"   # 각 워커마다 명시 (colon 포함 ref 를 구분자 파싱하지 말 것 — 규약 3)
deft-log "$SESSION_DIR" WAIT "$WORKER_NAME pane 쉘 readiness 대기 (최대 15s)"
# 규약 1: 입력창 클리어(C-u) 먼저. 규약: WS 배열(--workspace) 동반 필수(caller stale 폴백 방지 — RATIONALE R-8).
cmux send-key --surface "$SURF" "${WS[@]}" C-u
cmux send     --surface "$SURF" "${WS[@]}" "touch $SESSION_DIR/.ready-$WORKER_NAME"
cmux send-key --surface "$SURF" "${WS[@]}" Enter
...
> ⚠️ **진짜 codex 업데이트 프롬프트** (근거: RATIONALE R-12): 진짜 `codex`(claudex 아님)는 첫 기동 시 "Update available" 프롬프트로 정지할 수 있다. 감지되면 `cmux send --surface "$SURF" "${WS[@]}" "3"` + `cmux send-key --surface "$SURF" "${WS[@]}" Enter`(3=skip)로 넘긴다(`--workspace` 동반). 사전 `codex update` 로 예방.
...
# 규약 2: 긴 MCP 인라인 명령은 .sh 파일에 저장 → source 한 줄만 send (직접 send 하면 유실 — RATIONALE R-15).
echo "$WORKER_CMD" > "$SESSION_DIR/boot-$WORKER_NAME.sh"
cmux send-key --surface "$W1_SURFACE" "${WS[@]}" C-u                                   # 규약 1: 입력창 클리어
cmux send     --surface "$W1_SURFACE" "${WS[@]}" "source $SESSION_DIR/boot-$WORKER_NAME.sh"
...
# 규약 2: .sh 저장 → source 한 줄 (claude CLI 부팅도 길어 직접 send 시 유실 위험 — RATIONALE R-15).
echo "$WORKER_CMD" > "$SESSION_DIR/boot-$WORKER_NAME.sh"
cmux send-key --surface "$W1_SURFACE" "${WS[@]}" C-u                                   # 규약 1: 입력창 클리어
cmux send     --surface "$W1_SURFACE" "${WS[@]}" "source $SESSION_DIR/boot-$WORKER_NAME.sh"
cmux send-key --surface "$W1_SURFACE" "${WS[@]}" Enter
