...
> 🚨 **전송계층은 모드 종속이다 (실측 회귀 2026-06-25)**: **통신 모드(회의 vs 작업)가 전송계층 선택을 지배한다.** 아래 표는 *작업 모드* 기준이고, **회의 모드는 board 브로드캐스트가 본질이므로 무조건 MCP 버스를 쓴다**(claudex 0.139.1+ 가 있어도 NTP 네이티브 1순위를 적용하지 말 것). NTP 직접회수는 board(공유 진실 소스)를 만들지 못해 회의가 star 로 퇴화한다(실측 사고 — 회의를 NTP 로 띄워 워커 상호 노출이 사라짐). **회의의 속도는 버스의 `ntpPush` 전파**(cmux 노크 대신 팀 inbox 자동주입 — `multi-round-bus` 내장)가 책임지므로, NTP 네이티브 없이도 빠르다.

**회의 모드 (board 토론) — 무조건 버스:**

| 참가자 구성 | 1순위 | 폴백 |
|---|---|---|
| **회의 모드 전부** (claude/claudex/mix 무관, cmux 환경) | **MCP 버스** (board 공유 + `ntpPush` 빠른 전파 — §Phase 3-A) | send/capture (§Phase 3-B) → 불가 시 `multi-check` 안내 후 중단 |
| cmux 외부 환경 | claudex MCP conversation (§Phase 3-C) | `multi-check` 안내 후 중단 |
...
| **AI mix 또는 전원 claudex/codex** (cmux 환경) | NTP(claudex) / MCP 버스 | send/capture 폴백 | `multi-check` 안내 후 중단 |
...
> **근원 (실측+소스 확정 2026-06-25)**: NTP 는 송신·inbox 적재는 견고하나 **Lead 자동주입(수신)이 구조적으로 누락**된다. 원인은 *수신 측 turn 경계* 다 — 워커가 `team-lead.json` 에 정상 적재(고빈도 폴링으로 `read:false` 포착됨) → **Claude Code(Lead) 런타임 watcher 가 그 파일을 읽어 mailbox 큐에 넣고 즉시 비움** → 그러나 Lead 의 현재 turn 이 *이미 답변을 출력 중*(mailbox delivery phase = NextTurn)이면 그 큐가 이번 turn 에 주입되지 않고 **다음 turn 경계에서 누락** → inbox 파일은 이미 비워져 복원 불가 → 유실. (claudex watcher 는 자기 inbox 만 건드림 — 무죄. drain 주체는 Lead = Claude Code 런타임이고, 그 런타임은 클로즈드라 수정 불가.) 실측: 회의 4워커(claude2+claudex2) 입장 표명이 inbox 엔 적재됐으나 Lead transcript 자동주입 **0건**.

→ 그래서 **자동주입(`<teammate-message>`)을 1차 수신 경로로 신뢰하지 말 것.** Lead 는 워커에 `SendMessage` 한 **직후부터** `team-lead.json` 을 **직접 read 하여 회수**한다(자동주입은 우연히 오면 보너스인 보조 채널). 이게 turn 경계 누락을 **구조적으로 우회**한다 — Lead 가 Bash 로 직접 읽으므로 turn delivery phase 와 무관.

> 🚨 **순서가 핵심 — "회수 루프 먼저, SendMessage 나중" (실측 2026-06-25)**: 회수를 SendMessage **직후** 시작하면, 워커가 그 사이(수 초) 보낸 응답을 watcher(750ms)가 **먼저 비워** 놓친다(실측: 회수 루프를 늦게 켜니 입장 본문은 못 잡고 idle_notification 만 잡힘 → 회수 루프를 먼저 켜니 본문 회수 성공). 그래서 **반드시 회수 루프를 백그라운드로 먼저 띄운 뒤(`&`) → 그 다음 SendMessage** 한다.
...
> 🚨 **회의 모드 = MCP 버스 강제 (board 보장 가드 — 절대 우회 금지)**: 통신 모드가 **회의 모드**면 — claudex 0.139.1+ 가 있어도, 출력/속도 개선 어떤 이유로도 — **반드시 MCP 버스 경로(Phase 3-A)로 spawn 한다.** 회의의 본질은 board 공유(전원이 같은 단일 타임라인 참조)이고, 그 속도는 버스의 `ntpPush`(자동주입 전파)가 책임진다. **회의를 NTP 직접회수 경로로 띄우면 board 가 안 생겨 워커 상호 노출이 사라지고 star 로 퇴화**한다(실측 회귀 2026-06-25 — "토론" 요청이 1:1 분담 보고가 됨). 회의 spawn 후 **`board.jsonl` 생성을 반드시 확인**하고, 없으면 BLOCKED 처리 후 사용자 보고(가드). 작업 모드만 NTP 우선.
...
> 🚨 **이하 (2)~(5)의 모든 `cmux send`/`send-key`/`focus-pane`/`new-split`/`close-surface` 는 반드시 `--workspace "$LEAD_WORKSPACE"` 를 동반한다 (실측 잠복버그 — 2.5.0~2.40.0).** caller stale 환경(다른 워크스페이스에서 스킬 실행·resume 후·비대화형)에서 `--surface` 단독 ref 는 해석이 깨져 명령이 **Lead pane 으로 폴백 입력**된다(워커가 안 뜸 — touch·CLI 부팅·prompt 가 Lead 에 흘러들어감). NTP 헬퍼(`deft-claudex-native-spawn`)는 이미 이 패턴으로 고쳐져 있었으나(CHANGELOG 154) 버스 경로 send 엔 전파가 누락돼 있었다. **스킬은 어느 워크스페이스·어느 PC·어느 세션에서나 동작해야 하므로, 워크스페이스는 위 `cmux identify` 런타임 발견값(`$LEAD_WORKSPACE`)만 쓰고 특정 번호를 박지 않는다.**
...
```bash
# ⚠️ 모든 new-split 에 --workspace "$LEAD_WORKSPACE" 동반 (caller stale 대비 — 위 §가드).
WS=(--workspace "$LEAD_WORKSPACE")
SPLIT=$(cmux new-split right "${WS[@]}" --focus false 2>&1)                          # W1 (우측 새 컬럼)
W1_SURFACE=$(printf '%s' "$SPLIT" | grep -oE 'surface:[0-9]+' | head -1)
SPLIT=$(cmux new-split down --surface "$W1_SURFACE" "${WS[@]}" --focus false 2>&1)   # W2 (W1 아래로)
...
# ⚠️ **caller stale 시 down 대상 지정 우회**: `--surface "$W*_SURFACE"` 해석이 깨지면 "직전 워커 pane 을
#    cmux focus-pane --pane <prev> "${WS[@]}" → cmux new-split down "${WS[@]}""(focused 기준)으로 우회한다(NTP 헬퍼와 동일 패턴).
...
cmux focus-pane --pane "$LEAD_PANE" "${WS[@]}" 2>/dev/null   # Lead focus 복원 (--workspace 동반)
```
...
# ⚠️ --workspace "$LEAD_WORKSPACE"(WS 배열) 동반 필수 — 누락 시 caller stale 환경에서 touch 가 Lead pane 으로 폴백(실측 버그).
cmux send --surface "$W1_SURFACE" "${WS[@]}" "touch $SESSION_DIR/.ready-$WORKER_NAME"
cmux send-key --surface "$W1_SURFACE" "${WS[@]}" Enter
...
> ⚠️ **codex 워커 업데이트 프롬프트 (실측 — deft-test L4)**: 진짜 `codex`(claudex 아님)는 첫 기동 시 "Update available" 버전 업데이트 프롬프트를 띄워 회의 시작 전 정지할 수 있다 (claudex 는 자체 최신이라 안 뜸). spawn·readiness 확인 후 해당 프롬프트가 감지되면 `cmux send --surface "$SURF" "${WS[@]}" "3"` + `cmux send-key --surface "$SURF" "${WS[@]}" Enter` (3 = skip until next version) 로 넘긴다(`--workspace` 동반 필수). 사전에 `codex update` 로 최신화해 두면 예방된다.
...
cmux send --surface "$W1_SURFACE" "${WS[@]}" "$WORKER_CMD"
cmux send-key --surface "$W1_SURFACE" "${WS[@]}" Enter
```
...
cmux send --surface "$W1_SURFACE" "${WS[@]}" "$PERSONA_BOOT"
cmux send-key --surface "$W1_SURFACE" "${WS[@]}" Enter
...
- 🚨 **foreground blocking sleep 폴링 절대 금지 (실측 데드락 2026-06-25)**: Lead 는 응답을 기다릴 때 **`for i in $(seq ...); do sleep 30; ... done` 같은 foreground 대기 루프를 본체 턴에서 돌리지 말 것.** 그 Bash 가 끝날 때까지(예: 30초×16=8분) **모델 턴 전체가 블록**되어, 워커 응답·노크가 이미 도착해 있어도 Lead 가 다음 행동으로 못 넘어간다 → 무한 대기(사용자가 ESC 로 그 Bash 를 죽여야 큐에 쌓인 노크를 그제서야 처리 — 실측 사고). **올바른 대기**: ① 응답 확인은 **짧은 단발**(`check` 1회 또는 inbox read 1회)로 하고 **턴을 끝낸다** → 다음 노크/메시지가 다음 턴을 깨운다. ② 워커 무응답 감시가 필요하면 아래 데드락 워치독을 **반드시 `run_in_background`(또는 `&`)** 로 — foreground sleep 으로 대체하지 말 것. ③ "응답 올 때까지 기다린다"를 코드로 구현하려 하지 말 것 — 기다림은 **턴 종료 + 노크 재진입**으로 이뤄진다(Lead 가 능동으로 sleep 하는 게 아님).
...
- **본 회의 워커 pane(추적한 `W*_SURFACE`)만 닫는다**: `cmux close-surface --surface "$W1_SURFACE" "${WS[@]}"` … (워커 수만큼, `--workspace` 동반). 회의록은 board.jsonl·transcript.md 로 보존되므로 관찰 손실 없음.
