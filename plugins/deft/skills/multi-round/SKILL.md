...
### 절차
1. **팀 materialize + team-id 획득**: Lead 가 native `Agent` 1명을 spawn → 결과 `<name>@session-<id>` 에서 **team-session-id** 추출. (Claude 팀은 ≥1 Agent spawn 으로 생기고, 그 spawn 결과가 team-id 의 유일한 신뢰 출처 — 팀 config 의 `leadSessionId` 는 팀 자체 id 라 Claude 세션 id 와 다르므로 자동발견 불가, 실측.) claudex 만 워커로 쓰고 Claude 워커가 불필요하면, placeholder Claude Agent 로 team-id 만 얻고 즉시 shutdown 해도 된다.
2. **claudex 네이티브 기동**: `deft-claudex-native-spawn <team-id> <claudex-agent-name> [cwd]` (Phase 0 자동 설치). 설치 claudex 를 cmux pane 에 `--claude-team <team-id> --claude-team-agent <name> --dangerously-bypass-approvals-and-sandbox` 로 기동(readiness 가드 포함). 0.139.1 설치 전 테스트는 `CLAUDEX_NATIVE_BIN=<repo built 바이너리>` 로.
...
**용례 1 — team-id 획득 + Lead 가 팀원을 spawn**

claude 팀원 (첫 `Agent` spawn 이 팀을 materialize → 여기서 team-id 를 얻는다):

```
Agent(name:"<name>", subagent_type:"claude", model:"opus",
      description:"<한 줄>", prompt:"<task>")
# 반환된 "<name>@session-<id>" 의 session-<id> 가 곧 team-id.
# 멤버·leadSessionId 확인:
#   cat ~/.claude/teams/session-<id>/config.json
```

claudex 팀원 (NTP — 버스 대신 네이티브):

```bash
deft-claudex-native-spawn session-<id> <claudex-name> [cwd]
# cmux pane 에 claudex --claude-team session-<id> --claude-team-agent <claudex-name>
#   --dangerously-bypass-approvals-and-sandbox 로 기동(readiness 가드 포함).
...
- 팀원 응답은 `<teammate-message>` 로 **자동 주입**된다 — inbox 를 수동 확인하지 않는다.
- 자동주입 전제: 팀원이 **내 현재 팀**(용례 1 의 session-<id>)에 binding 돼 있을 것. 끊겼으면 team-lead.json 직접 확인으로 폴백.
- 종료: `SendMessage(to:"<name>", message:{type:"shutdown_request"})` → claudex 팀원은 watcher 가 직접 종료(pane 자동 close). 미종료 시 `pkill -f -- "--claude-team-agent <name>"` 폴백(소유 확인 후).
