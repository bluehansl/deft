...
skill 본체는 plugin marketplace cache(`~/.claude/plugins/cache/.../skills/multi-round/`)에 있으나, **skill 실행 시 사용하는 데이터·세션·hooks는 `~/.agents/skills/multi-round/`에 통일**한다 (다른 사용자 skill들과 일관).
...
│       ├── round2-<worker>.md
│       ├── ...
│       └── state.sh      # LEAD_SURFACE, W_*_SURFACE, W_*_PANE 등
...
```bash
SKILL_BASE="$HOME/.agents/skills/multi-round"
SESSION_TAG="$(date +%Y%m%d-%H%M)-<주제slug>"   # 예: 20260608-1430-api-design
SESSION_DIR="$SKILL_BASE/sessions/$SESSION_TAG"
mkdir -p "$SESSION_DIR"
```

**금지**:
- `/tmp/multi-round-session/` 같은 시스템 임시 경로 사용 금지 (재부팅 시 손실, 다른 skill과 경계 모호)
