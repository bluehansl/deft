...
- 🚨 **orca 모드에서 cmux CLI 호출 전면 금지** — 에러가 나는 게 아니라 **별도 cmux 앱의 엉뚱한 pane 을 조용히 조작**한다(오발사, 실측). deft bin 헬퍼(cmux-rebalancing/-guard/-watch·deft-claudex/claude-native-spawn·deft-cmux-shim·버스 cmuxKnock)는 orca 가드를 내장해 자체 차단하지만, **Lead 가 직접 치는 bare `cmux`/`tmux` 명령도 동일하게 금지**다.
...
| pane 정리 | `cmux close-surface` / `tmux kill-pane` | CLI 미확정 — shutdown 정상 종료(워커 자기종료) 우선, 잔존 pane 은 사용자 UI 수동 닫기 안내 |
...
- pane 환경(cmux 또는 orca — §환경 판정). orca 모드는 claudex 워커 기동을 헬퍼 대신 `orca terminal split/send/wait` 로 직접 수행한다(§운영 용례 1 의 🟠 orca 분기 — NTP 통신 자체는 파일 inbox 기반이라 환경 무관, 실측).
...
> 🟠 **orca 모드 (§환경 판정)**: 아래 골격 중 **cmux 전용 단계는 전부 skip** 한다 — ① rebalance-guard/`GUARD_FLAG` 발사·touch 불요(Orca 는 resize CLI 미지원 — bin 가드가 no-op. pane 비율은 UI 드래그 안내) ② `cmux list-panes` 로 `.last-worker-pane` 심기 불요(cmux 헬퍼 전용 상태파일) ③ claudex/claude-CLI 워커는 헬퍼(`deft-claudex/claude-native-spawn` — orca 가드로 차단됨) 대신 **orca terminal 로 직접 기동**한다:
>
> ```bash
> # ① 워커 pane 생성 (핸들 캡처·세부 플래그는 `orca skills get orca-cli` 가 단일 소스 — 실행 전 확인)
> orca terminal split --direction vertical
> # ② claudex 를 NTP binding 으로 부팅 — 헬퍼가 하던 기동 명령을 직접 send.
> #    회의 모드면 board 버스 인라인(-c 'mcp_servers.bus={…}')도 이 명령에 함께 포함(헬퍼의 DEFT_BUS_DIR 주입 등가).
> orca terminal send --text "claudex --dangerously-bypass-approvals-and-sandbox --claude-team $TID --claude-team-agent <name>" --enter
> # ③ TUI 부팅 대기 — readiness 마커 대신 orca 내장 idle 판정 활용(안정)
> orca terminal wait --for tui-idle --timeout-ms 60000
> ```
>
> 첫 claude 워커(Agent tool)는 orca claude-teams 의 tmux shim 이 pane 배치를 자동 처리하므로 **절차 변경 없음**. 회의 모드 board 등록(5)은 orca 모드에서 `--surface` 를 생략하고 **`--inbox` 만 등록**한다(cmux 노크 불가 — 깨우기는 ntpPush 전용, 파일 기반이라 환경 무관).
...
> 🟠 **orca 모드 (§환경 판정)**: 이하 (1)~(5)의 cmux 호출은 전부 금지(오발사) — ① (1)의 `cmux identify` 캡처(LEAD_SURFACE/LEAD_WORKSPACE) skip. Lead 등록은 (2) 첫 워커 spawn 으로 TID 확보 **후** `--surface` 대신 `--inbox ~/.claude/teams/$TID/inboxes/team-lead.json` 로 한다(워커 post 가 ntpPush 로 Lead 를 깨움 — 자동주입 유실 대비 무응답 시 `check --as lead` 수동 1회 병행). ② (3) `.last-worker-pane` 기록 skip. ③ (4) 헬퍼 대신 orca terminal 직접 기동(§용례 1 🟠). ④ (5-a) 워커 register 도 `--surface` 생략, `--inbox` 만.
...
- 첫 claude 워커 `Agent` tool spawn(팀 생성 겸 참가) → claudex 워커 `deft-claudex-native-spawn`(헬퍼 down. 🟠 orca 모드는 헬퍼 대신 orca terminal 직접 기동 — §용례 1 🟠). **§NTP 불변 하네스 H1~H4 그대로**(anchor/placeholder 금지).
- ⚠️ 회의와 유일한 차이: 헬퍼에 **`DEFT_BUS_DIR` 를 설정하지 않는다**(board 버스 미주입 = 순수 NTP. orca 모드 직접 기동 시에도 `-c mcp_servers.bus` 인라인을 넣지 않는다). claude 워커는 Agent tool 이라 어차피 버스 없음.
...
> 🟠 **orca 모드 (§환경 판정)**: 이하의 `close-surface`/`tmux kill-pane`/`cmux-rebalancing`/`focus-pane` 은 전부 금지(오발사·resize 미지원). 정리는 **shutdown 정상 종료만** 사용한다 — 첫 워커(Agent tool)는 `shutdown_request`→approved 대기(동일), 직접 기동한 claudex 워커도 `shutdown_request` 로 프로세스를 내리고(미종료 시 `pkill -f -- "--claude-team-agent <name>"` 폴백은 외부 프로세스라 orca 모드에서도 허용), **잔존 pane 은 사용자에게 UI 수동 닫기를 안내**한다(Orca pane 닫기 CLI 미확정 — `orca skills get orca-cli` 에 있으면 그것 사용). 레이아웃 복원 단계는 없음.
...
| orca 모드에서 bin 헬퍼가 "orca 모드 감지 — 차단" 반환 | 정상 가드 동작 — cmux 경로 재시도 금지, 해당 Phase 의 🟠 orca 분기로 진행 |
