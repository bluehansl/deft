...
> - 실제 작업(팀 spawn·페르소나 생성·pane 분할 등 능동 행위) 중에만 바쁘다. 그 외 — 특히 **팀원 응답을 기다리는 구간** — 은 **반드시 백그라운드/in-progress** 로 두고 Lead 본체는 **사용자 입력을 받을 수 있는 idle 상태**로 둔다.
> - 🚫 응답 대기를 **foreground blocking**(긴 `sleep` 루프, `wait`, 동기 폴링 등)으로 구현 **절대 금지** — 그 동안 Lead 가 마비돼 사용자 입력·워커 노크 모두 못 받는다(실측 데드락). 대기는 **짧은 단발 확인 후 턴 종료 → 노크/메시지가 다음 턴을 깨움**으로 이뤄진다.
>
> **원칙 2 — 사용자 대화 화면에는 "의미 이벤트"만 출력한다 (내부 메커니즘 전면 차단).**
> - ✅ **출력할 것 (최소한)**: 어떤 요청으로 / **몇 명이 어떤 페르소나로** 생성되는지 → 작업 시작 → **팀원 spawn 완료** → **회의 시작** → **회의 중간 결과·합의** → 최종 결론. 그게 전부다.
> - 🚫 **출력 금지 (전부 — `orchestration.log` 로만)**: CLI 부팅, 버스 등록, pane 분할/생성, 페르소나 주입, 헬퍼 동기화(deft-bin-sync), 세션 디렉토리 초기화, 레이아웃 정렬(Lead 60%/워커 40%·readiness), team-id, rebalance, 버전 게이트, "Ran N shell commands" 류 단계 중계.
> - 기준: 에이전트/cmux UI 가 pane·워커 상태를 이미 시각화하므로, 대화 텍스트는 **사용자가 궁금한 의미**(누가·무엇을·어떤 결과)만 담는다. 절차의 bash·헬퍼 호출은 **실행 지침**이지 읽어줄 대본이 아니다. (상세 §Lead 출력 레지스터 규약)
> - ✗ "헬퍼 동기화 및 세션 디렉토리 초기화합니다 / 레이아웃 정렬 완료 (Lead 60%) / 4개 pane 준비 완료, CLI 부팅 / 버스에 등록 완료, 페르소나 주입"
> - ✓ "물류·IT·VC·이커머스 4인 전문가 회의를 시작합니다." → (스폰 완료 후) "팀원 4명 준비 완료, 라운드 1 토론을 시작합니다." → (중간) "3인 합의·1인 이견 — 쟁점은 …"
...
COLLECT=<SESSION_DIR>/collected.jsonl   # 회수분 영속 — watcher 가 비워도 여기 보존
: > "$COLLECT"
EXPECTED_REPLIES=<기대 발신자 수>        # 예: 워커 4명 회의면 4

# 1) 회수 루프를 **백그라운드로 먼저** 띄운다 (SendMessage 보다 선행 — watcher 선점).
( for i in $(seq 1 120); do             # ~36초 (워커 응답 지연 고려, 필요시 연장)
    if [ -s "$LB" ] && [ "$(jq 'length' "$LB" 2>/dev/null)" -gt 0 ] 2>/dev/null; then
      # ⚠️ idle_notification·shutdown 등 제어 메시지는 회수 대상 아님 — 실제 보고 본문만.
      jq -c '.[] | select(.text != null and ((.text|type)=="string") and (.text|contains("idle_notification")|not))' \
        "$LB" 2>/dev/null >> "$COLLECT"
      jq 'map(.read=true)' "$LB" > "$LB.tmp" 2>/dev/null && mv "$LB.tmp" "$LB"   # 회수 후 마킹(중복 방지)
    fi
    # ⚠️ 기대 발신자 **전원**이 모일 때까지 break 금지(한 명 잡았다고 끊으면 나머지 놓침 — 실측 사고).
    [ "$(jq -r '.from' "$COLLECT" 2>/dev/null | grep -c . )" -ge "$EXPECTED_REPLIES" ] && break
    sleep 0.3
  done ) &
RECV_PID=$!
sleep 0.5   # 루프가 첫 회 돌기 시작하도록 짧게 양보

# 2) 이제 워커들에게 SendMessage (회수 루프가 이미 inbox 를 선점 감시 중).
#    SendMessage(to:"<worker>", ...) × N  ← 여기서 각 워커에 요청/의제 발송

# 3) 회수 완료 대기 후 결과 사용.
wait "$RECV_PID" 2>/dev/null
jq -r '"[\(.from)] \(.text // "")"' "$COLLECT" 2>/dev/null   # 실제 수신본 (자동주입 대화에 안 떠도 이게 진실)
```

- **적용 시점 (전부 — 자동주입 대기 자체를 생략)**: ① 회의 라운드/작업 지시 후 워커 응답 대기 ② mate↔mate 협의 추적 ③ 종료 시 `shutdown_approved` 대기(`type==shutdown_approved` envelope 를 inbox 에서 직접 확인).
