...
| 참가자 구성 | 1순위 | 2순위 | 3순위 |
|---|---|---|---|
| **Lead=Claude + claudex 워커 (claudex 0.139.1+ — 네이티브 binding 지원)** | **Claude 네이티브 inbox** (Lead 가 `SendMessage` 직접 — §claudex 네이티브 팀원) | **MCP 버스** | send/capture 폴백 |
...
## claudex 네이티브 팀원 (claudex 0.139.1+ — 버스 대신 네이티브 통신)
