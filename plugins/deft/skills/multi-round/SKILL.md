...
3. **통신**: Lead 는 `SendMessage(to:"<claudex-agent-name>")` 로 요청. claudex 는 자기 inbox 를 watch·드레인해 자기 턴 경계에 주입받고, `send_message` 도구(평문)로 답신 → Lead inbox → Lead 에 `<teammate-message>` 자동 주입.
   ...
   - ⚠️ **워커 보고 규칙 (출력 ≠ 전달 — 실측 확정·AI 무관)**: NTP 워커는 Lead 가 요청한 작업 결과를 — Lead 의 별도 지시가 없는 한 — **반드시 `send_message`(claudex) / `SendMessage`(claude) 로 보고**한다. 자기 세션에 출력만 하면 Lead 에 전달되지 않는다. 사용자가 워커 TUI 에 직접 친 것만 출력으로 답한다. (페르소나 `agents/*-participant.md` §보고 채널 원칙)
     ...
     - ✅ **Lead 직접 회수 강제 (§Lead 직접 회수 — 1차 수신 경로)**: Lead 는 자동주입을 1차 경로로 신뢰하지 말고, 워커에 `SendMessage` 한 **직후부터 `team-lead.json` 을 고빈도(0.5초 이하)로 직접 read 해 회수**한다(잡는 즉시 `COLLECT` 로 복사 — watcher 가 비워도 보존). turn 경계 누락을 Bash 직접 읽기로 구조적 우회. 상세 절차·근원은 §Lead 직접 회수.
...
- 팀원 응답은 `<teammate-message>` 로 자동 주입될 수 **있으나** 이를 신뢰하지 말 것 — turn 경계에서 누락돼 유실되는 사고가 잦다(§Lead 직접 회수). **Lead 는 SendMessage 직후 `team-lead.json` 을 직접 read 해 회수하는 것을 1차 경로로 삼는다.**
...
# ⚠️ watcher(750ms 주기)가 inbox 를 비우기 전에 선점해야 하므로, SendMessage 직후
#    "고빈도(0.5초 이하)" 로 짧게 회수 루프를 돈다. 잡은 메시지는 즉시 COLLECT 로 복사(유실 방지).
for i in $(seq 1 60); do                      # ~30초 (워커 응답 지연 고려, 필요시 연장)
  if [ -s "$LB" ] && [ "$(jq 'length' "$LB" 2>/dev/null)" -gt 0 ] 2>/dev/null; then
    jq -c '.[] | select((.type//"message")=="message" or .text!=null)' "$LB" 2>/dev/null >> "$COLLECT"
    jq 'map(.read=true)' "$LB" > "$LB.tmp" 2>/dev/null && mv "$LB.tmp" "$LB"   # 회수 후 마킹(중복 방지)
  fi
  # 기대한 발신자가 다 모였으면 break (예: 4명 전원). COLLECT 의 .from uniq 로 판정.
  [ "$(jq -r '.from' "$COLLECT" 2>/dev/null | sort -u | wc -l)" -ge "$EXPECTED_REPLIES" ] && break
  sleep 0.5
done
# 회수 결과로 진행 — 자동주입 대화에 안 떠도 COLLECT 가 실제 수신본이다.
jq -r '"[\(.from)] \(.text // .message // .summary // "")"' "$COLLECT" 2>/dev/null
```
...
- **고빈도가 핵심**: watcher 가 750ms 마다 비우므로, 30초 같은 **느린 폴링은 빈 inbox 만 보고 "유실"로 오판**한다(실측 — 그래서 claude-2.37.0 의 "느린 폴링 권고"로는 부족했다). SendMessage 직후 **0.5초 간격으로 선점 회수**하고 잡는 즉시 `COLLECT` 로 복사한다.
