...
3. **통신**: Lead 는 `SendMessage(to:"<claudex-agent-name>")` 로 요청. claudex 는 자기 inbox 를 watch·드레인해 자기 턴 경계에 주입받고, `send_message` 도구(평문)로 답신 → Lead inbox → Lead 에 `<teammate-message>` 자동 주입.
   ...
   - ⚠️ **워커 보고 규칙 (출력 ≠ 전달 — 실측 확정·AI 무관)**: NTP 워커는 Lead 가 요청한 작업 결과를 — Lead 의 별도 지시가 없는 한 — **반드시 `send_message`(claudex) / `SendMessage`(claude) 로 보고**한다. 자기 세션에 출력만 하면 Lead 에 전달되지 않는다. 사용자가 워커 TUI 에 직접 친 것만 출력으로 답한다. (페르소나 `agents/*-participant.md` §보고 채널 원칙)
     - 🚨 **`success:true` ≠ 전달 성공 (실측 2026-06-25)**: claude·claudex 워커 모두, 송신 도구가 `Message sent to … inbox` / `success:true` 를 반환해도 **Lead inbox(`team-lead.json`)에 0건으로 안 남고 자동 주입도 안 오는** 사고가 실측됐다(porter 수신 사고 — claude 워커 `SendMessage` 2회 모두 success 였으나 inbox 0건). 송신 측 성공 신호는 **inbox 적재 시도까지만** 보장하며 Lead 수신을 보장하지 않는다.
     - ✅ **Lead 능동 폴링 강제 (§폴백 워치독 — 권고 아님, 필수)**: Lead 는 워커 응답을 `<teammate-message>` **자동 주입에 의존하지 말고**, 응답 대기 구간에서 **`team-lead.json` inbox 를 직접 폴링**한다. 워커가 "보냈다"고 해도 Lead 가 **실제 결과물(중간 산출·결론)을 받기 전엔 미수신으로 간주**하고, 타임아웃 내 미도착이면 워커에 **재보고를 요청**한다(claudex=`SendMessage(to:"<워커>")` 로 "결과를 send_message(target:\"team-lead\", …) 로 다시 보내라"). 자동 주입은 best-effort 보조 채널일 뿐이다.
