...
| `deft:multi-round` | **여러 AI가 N라운드 양방향으로 토론·합의 도달** 하는 멀티턴 회의. broker 없이 `claudex mcp-server` 경유. [상세 가이드](skills/multi-round/GUIDE.md) |
...
| `multi-round` | **지속 N라운드 양방향** | Claude + Claudex mix | **MCP 경유**, cmux·팀기능 무관 | "의견 갈리는 문제 토론해서 합의" |
| Claude Code Agent Teams | 지속 multi-turn | Claude끼리만 | Claude 팀 기능 베이스 | "코드 분담 구현·PR 작업" |
