...
| 항목 | 구분 |
|---|---|
| Claude Code (플러그인 지원 버전) | 필수 |
| cmux | 선택 — `multi-round`/`multi-check` 워커 pane 시각화 |
| codex·gemini CLI | 선택 — `multi-check` 3-AI 비교용 |
...
`/plugin` 관리 화면에 `deft` 가 설치됨으로 보이면 (또는 새 세션 자동완성에 `/set-statusline` 등 deft 스킬이 뜨면) 설치 완료.

**첫 실행 예제 — `/set-statusline`** (저비용 · 결과 즉시 확인 · 원복 내장)

```
/set-statusline
```

- **성공 기준**: 의존성 점검 결과가 출력되고, `y` 컨펌 후 터미널 하단 statusline 이 4-segment(📁 폴더 / git branch / 🤖 모델 / ctx 진행바)로 표시된다.
- **실패 시**: `/restore-statusline` 으로 설치 직전 상태 원복. 그 외 증상은 아래 고지 섹션 2개(워커 권한 모드 / 세션 바이너리 keepalive)와 [CHANGELOG.md](CHANGELOG.md) 의 최근 Fixed 항목을 확인.

**버전·변경 이력** — 버전 숫자는 본 문서에 고정 기재하지 않는다.

- 현재 설치 버전: [.claude-plugin/plugin.json](.claude-plugin/plugin.json) 의 `version` · 변경 이력: [CHANGELOG.md](CHANGELOG.md)
- 버전 의존 동작은 도입 버전을 병기한다 — 예: `multi-round` 기본 워커 3명은 `claude-2.13.0+`.
...
| `deft:multi-round` | **여러 AI(Claude/Claudex)가 두 통신 모드로 협업하는 멀티턴**. ① **회의 모드** — N라운드 양방향 토론으로 합의 도달(board 브로드캐스트 + 노크). ② **작업 모드** — board 없는 NTP mesh 로 일 분담(Lead↔mate 1:N + mate↔mate N:N, work.md 취합). 기본 워커 3명·주제 맞춤 페르소나(`claude-2.13.0+`). 회의 워커는 **NTP 이름표 + board 토론 2채널 공존**(`claude-2.43.0+`). [상세 가이드](skills/multi-round/GUIDE.md) |
...
| `multi-round` | **회의: N라운드 양방향 / 작업: NTP mesh** | Claude + Claudex mix | **board 버스 + NTP + cmux pane** | "의견 갈리는 문제 토론·합의 / 일 분담" |
...
- 이미 바이너리가 삭제된 오래된 세션은 최신 버전으로 **대체 복원** (spawn 동작 보장 — 부모·자식 버전 불일치 가능성 1회 고지). 완전 정합을 원하면 해당 세션 재시작(`/resume`)
- **SessionStart hook (claude-2.15.0+)**: 본 플러그인은 세션 시작 시 keepalive 를 자동 실행하는 hook 을 포함한다 — 스킬 실행 여부와 무관하게 새 세션이 뜰 때마다 그 시점의 전 버전이 보존된다 (1초 미만, 상주 프로세스 없음). 원치 않으면 `hooks/hooks.json` 의 SessionStart 항목 제거
